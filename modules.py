...
def normalize(inputs, 
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        beta= tf.Variable(tf.zeros(params_shape)) # beta和gamma随网络一起训练
...
def shift_concat(decoder_outputs, fake_tensors, time_major=False):
    ...
    if fake_tensors is not None:
        ...
        fw_ft = tf.zeros_like(fw_outputs[0:1]) if time_major else tf.zeros_like(fw_outputs[:, 0:1]) # [max_time, batch_size, num_units]
...
def multihead_attention(queries, 
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # Linear projections
        Q = tf.layers.dense(queries, num_units, activation=tf.nn.relu) # (N, T_q, C) 为什么还会有relu函数？？？
...
def bidirectional_lstm_encoder(inputs, num_units=128, scope="bi_lstm"):
    lstm_fw_cell = tf.contrib.rnn.BasicLSTMCell(num_units)
    lstm_fw_cell = tf.contrib.rnn.DropoutWrapper(cell=lstm_fw_cell, output_keep_prob=0.5)
    lstm_bw_cell = tf.contrib.rnn.BasicLSTMCell(num_units)
    lstm_bw_cell = tf.contrib.rnn.DropoutWrapper(cell=lstm_bw_cell, output_keep_prob=0.5)
...
def label_smoothing(inputs, epsilon=0.1):
    ...
    return ((1-epsilon) * inputs) + (epsilon / K) # pytorch的代码好像除的是K-1 ？？？
